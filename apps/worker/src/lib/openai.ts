...
export interface KickoffMessageParams {
    env: { OPENAI_API_KEY: string };
...
/**
 * Generates a kickoff (AI-first) opening message for a scenario.
 * Used when a scenario has kickoff.enabled === true.
 * The AI generates a dynamic in-scene opening based on the kickoff prompt.
 *
 * @returns The generated opening message text
 * @throws If the OpenAI call fails
 */
export async function generateKickoffMessage(params: KickoffMessageParams): Promise<string> {
    const apiKey = params.env.OPENAI_API_KEY;
    if (!apiKey) {
        throw new Error('OPENAI_API_KEY is not set in Worker env');
    }

    const systemPrompt = `You are a language tutor playing a character. Generate an opening message to start a lesson.
...
KICKOFF INSTRUCTION:
${params.kickoffPrompt}

IMPORTANT:
- Output ONLY the opening message text, nothing else.
- Do NOT include any JSON, metadata, or formatting.
- Stay fully in character.
- This is the very first message the student sees — make it welcoming and natural.`;
    ...
    if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`OpenAI API error during kickoff: ${response.status} ${errorText}`);
    }

    const data: OpenAIResponse = await response.json();

    if (!data.choices || data.choices.length === 0) {
        throw new Error('OpenAI kickoff response has no choices');
    }

    const content = data.choices[0]?.message?.content?.trim();

    if (!content) {
        throw new Error('OpenAI kickoff response has no content');
    }
